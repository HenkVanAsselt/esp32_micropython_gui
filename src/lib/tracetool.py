...
try:
    import queue as Queue         #IGNORE:F0401   # python 3    #@UnresolvedImport #@UnusedImport
except Exception as importError:  #IGNORE:W0703
    import queue                                  # python 2    #@Reimport
...
# noinspection PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone
class TraceToSend(TraceNodeBase):
    ...
    def getLastContext(self):
        ...
        # no need to lock an empty listctrl
        if cList.count == 0:
            return None
...
# noinspection PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone
class TraceNodeEx(TraceNodeBase):
    ...
    def _innerAddValue(self,objToSend, upperNode, maxLevel, sendPrivate, alreadyParsedObject):
        ...
        try:
            ...
            try:
                #/ this is the only place where object is added to alreadyParsedObject listctrl
            ...
            if isinstance(objToSend,dict) or type(objToSend) is type(type.__dict__):    # types.DictProxyType
                c = 0
                for key in list(objToSend.keys()):
                    if (key == None):
                        title = "[None]"
                    else:
                        title = "[" +str(key) + "]"
    ...
    def addTable(self,table):
        ...
        #innerAddTable( TableMembers,  listctrl,  isFirst)
    ...
    def _innerAddTable(self,tableMembers, rowObject, rowNum):
        ...
        try:
            ...
            # Map
                ...
                checkMapInHeader ()
                for fieldName in list(rowObject.keys()):
...
# noinspection PySimplifyBooleanCheck,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone
...
class TTrace(object):
    ...
    # noinspection PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PySimplifyBooleanCheck,PyComparisonWithNone,PyComparisonWithNone,PyComparisonWithNone
...
##            self.connection_refused = False
        ...
        def getObjectInfo(objToSend):   # TTrace.Utility().getObjectInfo(objToSend)
            ...
            # Same as str() for listctrl but don't add [] charaters
            ...
            try:
                ...
                #listctrl, tuple, set, dict content don't have to be displayed here. Just display the count
                #------------------------------------------------------------------------------------
                ...
                elif objClass is list:                                                  # <type 'listctrl'>
                    memberType = "List"
                    memberValue = "count=%d" %(len(objToSend))
                ...
                if isFunction == True:
                    ...
                    if hasattr(objToSend, 'func_code'):
                        info = inspect.getargs(objToSend.__code__)
                        if hasattr(objToSend, 'func_name'):
                            memberValue = objToSend.__name__ + "(" + listToString(info.args) + ")"
                        else:
                ...
                # because the doc come from the result. Result is just stored in a listctrl !!!
                #-------------
        ...
        def uuid():  # TTrace.Utility().uuid()
            ...
            return str(uid) + str(perf_counter()) + str(random.randrange(0, 9999))
        ...
        def sendToWinTraceClient(commandList, winTraceId, dateTime = None, threadName = None):   # TTrace.Utility().sendToWinTraceClient(...)
            ...
            @param commandList : The message listctrl to send
...
##                return
            ...
            try:
                ...
                ttrace.warning.enabled = False
                ttrace.error.enabled = False
##                tr =   inspect.trace() [0]
##                print ( "ttrace exception : " + str(tr[3]) + "(" + str(tr[2]) + ")" , e)
...
if __name__ == '__main__':
    ...
    if _Internals.pyVersion <= 2:
        ttrace.debug.sendDump("hello", "worsdgsdgdsfgfdvhfcsdscvfghjjhgfjbjfg&é§è§!çàëêld")
    else:  # some unicode characters
        ttrace.debug.sendDump("hello", "worsdgsdgdsfgfdvhfcsds" + chr(500) + chr(1000) + chr(2000) + chr(60000) +  " cvfghjjhgfjbjfg&é§è§!çàëêld")
