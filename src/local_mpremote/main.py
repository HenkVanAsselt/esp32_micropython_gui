...
from .pyboard import get_last_output, reset_last_output
    ...
    try:
        while args:
            ...
            if cmd == "disconnect":
                ...
                reset_last_output()     # Clear buffer in which the last output was written
                follow = True
                if args[0] == "--no-follow":
                    args.pop(0)
                    follow = False
                ...
                # print(f"after execbuffer(), {ret=}, {get_last_output()=}")
                if ret:
                    return ret
            elif cmd == "fs":
